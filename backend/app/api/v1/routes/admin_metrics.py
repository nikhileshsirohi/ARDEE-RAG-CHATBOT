...
from app.schemas.rag import BotTokenUsageMetric, DailyTokenUsageMetric, UserTokenUsageMetric
...
@router.get(
    "/token-usage/bots",
    response_model=list[BotTokenUsageMetric],
    summary="List per-bot token usage metrics",
)
async def list_bot_token_usage_metrics(
    _admin_user: AdminDep,
    repository: MetricsRepositoryDep,
    start_at: Annotated[datetime | None, Query()] = None,
    end_at: Annotated[datetime | None, Query()] = None,
    ...
    """Return per-bot token usage totals for the admin dashboard."""
    if start_at is not None and end_at is not None and start_at > end_at:
        raise BadRequestError("start_at must be before or equal to end_at")

    return await repository.list_bot_token_usage(
        start_at=start_at,
        end_at=end_at,
        limit=limit,
        offset=offset,
...
async def list_daily_token_usage_metrics(
    ...
    """Return per-day token usage buckets, optionally scoped to a user or bot.
